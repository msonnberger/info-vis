...
const routesContent = await readFile('../routes.csv', 'utf-8');
const routes = parseSync(routesContent, { columns: routesColumns, from_line: 2 });
const routeIds = routes.map((route) => route.route_id);

const readStream = createReadStream('../raw/trips.csv', 'utf-8');
const writeStream = createWriteStream('../trips.csv', 'utf-8');
